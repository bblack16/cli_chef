class Handbrake < CLIChef::Cookbook
  ...
  def reencode input, output, **args
    ...
    run(args) do |line, stream|
      self.process_encode_line(line, stream) rescue nil
    end
  end
  ...
  protected
    ...
    def setup_defaults
      ...
      add_ingredient(
        ...
        { name: :encoder, description: 'Set video library encoder. Options: x264/x265/mpeg4/mpeg2/VP8/theora', flag: '--encoder', allowed_values: ['x264', 'x265', 'mpeg4', 'mpeg2', 'VP8', 'theora'], aliases: [] },
        ...
        { name: :audio_encoder, description: 'Audio encoder(s): av_aac, fdk_aac, fdk_haac, copy:aac, ac3, copy:ac3, copy:dts, copy:dtshd, mp3, copy:mp3, vorbis, flac16, flac24, copy. copy:* will passthrough the corresponding audio unmodified to the muxer if it is a supported passthrough audio type. Separated by commas for more than one audio track.', flag: '--aencoder', allowed_values: ['av_aac', 'fdk_aac', 'fdk_haac', 'copy:aac', 'ac3', 'copy:ac3', 'copy:dts', 'copy:dtshd', 'mp3', 'copy:mp3', 'vorbis', 'flac16', 'flac24', 'copy'], aliases: [] },
        { name: :audio_copy_mask, description: 'Set audio codecs that are permitted when the \'copy\' audio encoder option is specified (aac/ac3/dts/dtshd/mp3, default: all).', flag: '--audio-copy-mast', allowed_values: ['all', 'aac', 'ac3', 'dts', 'dtshd', 'mp3'], aliases: [] },
        { name: :audio_fallback, description: 'Set audio codec to use when it is not possible to copy an audio track without re-encoding.', flag: '--audio-fallback', allowed_values: ['av_aac', 'fdk_aac', 'fdk_haac', 'copy:aac', 'ac3', 'copy:ac3', 'copy:dts', 'copy:dtshd', 'mp3', 'copy:mp3', 'vorbis', 'flac16', 'flac24', 'copy'], aliases: [] },
        ...
        { name: :audio_dither, description: 'Apply dithering to the audio before encoding. Separated by commas for more than one audio track.  Only supported by some encoders (fdk_aac/fdk_haac/flac16).  Options: auto (default),  none,  rectangular,   triangular,   triangular_hp,  triangular_ns', flag: '--adither', allowed_values: ['auto', 'none', 'rectangular', 'triangular', 'triangular_hp', 'triangular_ns', String], aliases: [] },
        ...
        { name: :crop, description: 'Set cropping values (default: autocrop)', flag: '--crop', allowed_values: [], aliases: [] },
        ...
        { name: :color_matrix, description: 'Set the color space signaled by the output. Values: 709, pal, ntsc, 601 (same as ntsc) (default: detected from source)', flag: '--color-matrix', allowed_values: [709, 'pal', 'ntsc', '601'], aliases: [] },
        { name: :deinterlace, description: 'Unconditionally deinterlaces all frames  <fast/slow/slower/bob> or omitted (default settings)  or  <YM:FD>  (default 0:-1)', flag: '--deinterlace', allowed_values: ['omitted', 'fast', 'slow', 'slower', 'bob'], aliases: [] },
        ...
        { name: :nimeans_tune, description: 'Tune nlmeans filter to content type. Note: only works in conjunction with presets ultralight/light/medium/strong. <none/film/grain/highmotion/animation> or omitted (default none)', flag: '--nlmeans-tune', allowed_values: ['none', 'film', 'grain', 'highmotion', 'animation', 'omitted'], aliases: [] },
    ...
    def process_encode_line line, stream
      begin
